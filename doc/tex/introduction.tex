...
\begin{frame}[c,fragile]
  ...
  \item Three steps:
    ...
    \item[--] solve each block by algebra: a single equation is rearranged to give
      its variable; a small block is reduced by substituting one equation into the
      others, and solved exactly when it is linear in its unknowns.
    \end{itemize}
...
\begin{frame}[fragile]
  ...
  \medskip
  A linear block is solved exactly. The \verb+a+, \verb+b+ process of the earlier
  model gives
  \[
