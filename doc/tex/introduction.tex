...
\begin{frame}[fragile]
  ...
  \medskip
  An equation may stand for a variable it mentions. Three cases, from best to worst:
  \begin{enumerate}\small
  \item the variable is still there once leads and lags are dropped and the equation
    simplified: \texttt{y = alpha*k(-1) + e} may stand for \texttt{y} or for \texttt{k};
  \item it survives only as a common factor: \texttt{junk = 0.9*junk(+1)} reduces to
    $\texttt{junk}\cdot 0.1 = 0$, which fixes \texttt{junk} at zero. But an Euler
    equation reduces to $c^{-\sigma}(1 - \beta R) = 0$, which pins $R$ and leaves $c$
    free: whether such an equation is the variable's depends on the other factor, and
    the pairing only ranks the case below the first;
  \item it disappears altogether: \texttt{Y/Y(-1) = g} reduces to \texttt{1 = g}, yet
    this is the equation that gives the level of \texttt{Y}.
  \end{enumerate}

  \medskip
  Among the variables an equation may stand for, the one on its left-hand side comes
  first, then one that few other equations could take, so that it is not left stranded.
...
\begin{frame}[fragile]
  ...
  Each possible pair is given a price: the case number, a discount for the left-hand
  side, a small surcharge when many equations want the same variable. Leaving an
  ...
  \begin{tabular}{l|cccc}
    ...
    \texttt{junk = 0.9*junk(+1)}     &            &            &            & \textbf{2}    \\
  \end{tabular}
  \end{center}
  \small The case of each possible pair, in bold the assignment chosen: the left-hand
  side settles the first three rows, and the last equation takes \texttt{junk} although
  it only holds it as a factor, since nothing else can.
