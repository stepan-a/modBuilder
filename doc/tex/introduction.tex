...
\begin{frame}[c,fragile]
  ...
  \item The level of \verb+a+ is not declared: the plan closes it from its own
    process, \verb+a = rho*a(-1) + e+, at zero.\newline

  \item \verb+PropagateKnown+ inlines what is already known -- here $h=1/3$, set by
    \verb+calibrate+, and $e=0$ -- before the recognisers run.\newline
