...
\begin{frame}[fragile]
  ...
  \item it survives only as a common factor: \texttt{junk = 0.9*junk(+1)} reduces to
    \texttt{0.1*junk = 0}. That does fix \texttt{junk}, at zero, but the steady-state
    solver treats levels as non-zero, it divides by them and cancels them, and sets
    such an equation aside; it is plainly the equation of \texttt{junk} all the same;
