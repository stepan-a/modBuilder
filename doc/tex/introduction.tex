...
model.solve_system({'a', 'b', 'y', 'c', 'k', 'h'}, ...
  ...
  \item \verb+solve_system+ solves $n$ static equations for $n$ symbols jointly, by
    ...
    variables, exogenous variables and parameters, here \verb+psi+, which starts
    from its calibrated value.
  \item The block of \verb+a+ and \verb+b+, which one equation at a time could not
    untangle, is solved with the rest: $a = b = 0$.
