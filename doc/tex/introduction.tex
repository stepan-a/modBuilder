...
\begin{frame}[fragile]
  \frametitle{Instantiate a model from a \texttt{.mod} file -- the matching --}

  Which variable is each equation \emph{for}? A bipartite graph joins an equation to
  every endogenous variable it mentions, at a cost that ranks how the equation holds it:

  \begin{enumerate}\small
  \item the static residual pins it down: \texttt{y = alpha*e} $\to$ \texttt{y - alpha*e};
  \item it only factors out of the static residual: \texttt{junk = 0.9*junk(+1)} $\to$
    \texttt{0.1*junk};
  \item only a lead or a lag carries it: \texttt{Y/Y(-1) = g} $\to$ \texttt{1 - g}.
  \end{enumerate}

  Within a tier, a variable on the left-hand side is cheaper, then one few equations
  could take. Leaving an equation unmatched costs more than any edge, so the matching
  is as large as it can be first, and as cheap as it can be then (\texttt{matchpairs},
  Duff and Koster). A perfect matching keys every equation; none means the error.
  ...
  \end{tabular}
  \end{center}
  \scriptsize Tiers of each edge; in bold the pairing chosen, the left-hand side deciding
  the ties. The steady-state plan keeps its own matcher, which admits tier 1 only: it
  asks which variable an equation pins down at the steady state, not which it is for.
