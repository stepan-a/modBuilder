...
\begin{frame}[fragile]
  ...
  The macro language is read in full, and its control flow survives into the script
...
\begin{frame}[fragile]
  ...
  tag that is a declared variable settles it; the rest are paired by a bipartite
  matching that asks which variable each equation is \emph{for}: a variable the static
  residual pins down comes first, then one it holds as a factor, then one only the leads
  and lags carry, so \mbox{\texttt{junk = 0.9*junk(+1)}} or \mbox{\texttt{Y/Y(-1) = g}}
  get their variable too. When no perfect pairing exists, the error names what to tag:

  \begin{lstlisting}[style=MatlabConsole]
    >> modBuilder('twoeqs.mod')
Error using modfile.name_equations
twoeqs.mod: unable to associate every equation
with a unique endogenous variable.
Add a "name" tag to these:
  line 7: y = 2*e
  unmatched endogenous variables: c
  \end{lstlisting}

  \medskip
  Of the 280 files of Dynare's test suite that use the macro language, 222 load as
  they are. Most of the rest hit a refused statement, an operator the parser does not
  know yet (\texttt{$\perp$}), or a \texttt{-D} flag the test runner supplies.
