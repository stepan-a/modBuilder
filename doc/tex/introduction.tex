...
model =
  ...
  \item The other route stays available when Dynare has already run the file:
    \verb+modBuilder(M_, oo_, 'model.json')+ builds the model from what the session holds.
  \end{itemize}
...
m.add('h', 'h = theta*y');
  ...
  read with its \verb+name+ tags, or paired as described earlier. To re-pair without
  removing anything, \verb+reassign+ cycles the association of several equations at once.
...
%==============================================================================
% Inspecting, composing and comparing models
%==============================================================================

\begin{frame}[c,fragile]
  \frametitle{Inspect a model}
...
id =
  ...
  \begin{itemize}\small
  \item \verb+table('parameters')+ returns the same content as a matlab table, with the
    long names and the \TeX{} names; \verb+equationmap+ prints which equation each variable
    is keyed to, \verb+size+ counts, \verb+getallsymbols+ lists.
  \item \verb+isparameter+, \verb+isexogenous+, \verb+isendogenous+ and \verb+issymbol+
    answer the same question as \verb+typeof+ with a logical. Reading a symbol is
    \verb+model.alpha+, writing one is \verb+model.beta = 0.95+.
  \end{itemize}
...
\begin{frame}[c,fragile]
  \frametitle{Compose and compare models}

  \begin{lstlisting}[style=MatlabConsole,basicstyle=\ttfamily\tiny]
...
ans =
  ...
  \begin{itemize}\small
  \item \verb+extract+ takes equations by name, \verb+select+ by tag -- the two submodel
    constructors behind \verb+m{...}+ and \verb+m{bytag(...)}+.
  \item \verb+copy+ is not decoration: \verb+modBuilder+ is a handle class, so \verb+m2 = m+
    aliases the same model -- every trial in \verb+suggest_calibrations+ runs on a copy.
  \item \verb+==+ compares structure and calibration, ignoring long names and \TeX{} names:
    a refactor can be checked against a reference model.
  \item \verb+merge+ builds one model out of two, as the next frames do with two agents.
  \end{itemize}
...
model.write('rbc', steady_state_model=true, steady=true);
  ...
  \item Two more \verb+write+ options: \verb+check=true+ adds Dynare's \verb+check+ after
    \verb+steady+, and \verb+precision=+ sets how many digits the written calibration carries.
  \end{itemize}
...
\begin{frame}[c,fragile]
  ...
  \item Or solve within \verb+modBuilder+:
    \begin{itemize}
    ...
    \item[--] \verb+solve_system+ solves several equations for as many symbols at
      once.
    \end{itemize}
...
\begin{frame}[c,fragile]
  ...
  \begin{itemize}\small
  \item Each \verb+solve+ handles one static equation for one symbol, simplified
    first; the order of the calls does the rest, as by hand.
  \item The equation of \verb+k+ needs \verb+a+ and \verb+b+, which enter it through
    \verb|exp(a)*exp(b)|, and their own two equations each need the other: one
    equation at a time cannot untangle them, so their level, zero, is given
    (\verb+solve_system+ can, next frame).
    \verb+c+ needs no value: it cancels from the static equation of \verb+k+.
...
\begin{frame}[fragile]
  \frametitle{Steady state -- a system at once --}
...
model.solve_system({'a', 'b', 'y', 'c', 'k', 'h'}, ...
  ...
  \begin{itemize}\small
  \item \verb+solve_system+ solves $n$ static equations for $n$ symbols jointly, by
    Newton's method from their current values; the symbols can mix endogenous
    variables, exogenous variables and parameters, here \verb+psi+.
  \item Any subset of the equations will do, the other symbols held at their
    values: \verb+solve_system({'a', 'b'}, {'a', 'b'})+ solves the block that one
    equation at a time could not untangle, $a = b = 0$.
  \item Same point as the equation-by-equation route, $\psi = 0.1213$ and
    $k = 12.66$, within the default tolerance $10^{-6}$ (option \verb+tol+).
  \item \verb+Method+ chooses how the Jacobian is obtained: automatic
    differentiation by default, or analytical partials, or finite differences.
  \end{itemize}
...
\begin{frame}[c,fragile]
  ...
  \item Three steps:
    ...
    \item[--] order the variables: an equation depends on the variables it
      mentions; variables whose equations need one another, in a loop, form a
      \textbf{block} solved together, and the blocks are taken in an order where
      each uses only what the previous ones gave;
    \item[--] solve each block by algebra: a single equation is rearranged for its
      variable; a small block is reduced by substituting its equations into one
      another. A linear block always yields, unless its determinant is zero; a
      nonlinear one does when the rearrangements succeed (listed later), and
      otherwise resists.
    \end{itemize}
...
\begin{frame}[fragile]
  \frametitle{Steady state plan -- reading the plan --}

  The five blocks of the previous frame's plan, in the order they are solved:
  \begin{enumerate}\small
  \item \verb+a+, from its own process, \verb|a = rho*a(-1) + e|: $a = 0$.
  \item \verb+y+ and \verb+k+ together. Once $c/c(+1)$ is one, the Euler equation
    reads $\alpha y/k = 1/\beta - 1 + \delta$, and the production function also
    links $y$ and $k$: each equation needs the other. Substituting the production
    function into the Euler equation leaves one equation in $k$; $y$ follows.
  \item \verb+i+ from capital accumulation, $i = \delta k$.
  \item \verb+c+ from the resource constraint, $c = y - i$.
  \item $\theta$ from labour supply, hours being fixed at $1/3$.
  \end{enumerate}
  \small Blocks 3 to 5 are single equations, each rearranged for its variable.

  \medskip
  A linear block always yields a closed form, unless its determinant is zero. The
  \verb+a+, \verb+b+ process of the earlier model gives
  \[
    b = \frac{\tau e + (1-\rho)\, u}{(1-\rho)^2 - \tau^2}, \qquad a = \frac{e + \tau b}{1-\rho},
  \]
  whose denominator is the determinant of the block: the plan reports a unit root
  when it is zero at the calibration.

\end{frame}


\begin{frame}[fragile]
  \frametitle{Steady state plan -- the rearrangements --}

  A single equation is rearranged for its unknown $x$ when, once simplified, $x$
  enters it in one of four ways, its leads and lags counting as $x$:
  \begin{itemize}\small
  \item linearly, $a x + b = 0$, so $x = -b/a$: $i = \delta k$ from
    $k = (1-\delta)k + i$;
  \item as one power, $a x^d + b = 0$, so $x = (-b/a)^{1/d}$: $k$ from
    $\alpha k^{\alpha-1} h^{1-\alpha} = 1/\beta - 1 + \delta$;
  \item as two powers, $c_p x^p + c_q x^q = 0$, so $x = (-c_q/c_p)^{1/(p-q)}$;
  \item inside \verb+exp+, \verb+log+ or \verb+sqrt+: the function is inverted
    first, \verb+sqrt+ by squaring, so $\log Z = \rho \log Z + e$ gives
    $Z = \exp\left(e/(1-\rho)\right)$.
  \end{itemize}

  \small When $x$ sits in a denominator, the equation is first multiplied through by
  it. Anything else, $\log y + y^3 = x$ for instance, has no closed form: the block
  resists, and is left to a numerical solve (later frame).

  \medskip
  Each case returns \textbf{one} closed form, never a set of roots: nothing branches, and no
  root is tested afterwards. Where several solutions exist the branch is fixed by convention:
  $(-b/a)^{1/d}$ is the principal root, so $x^2 = 4$ gives $2$ and not $-2$; the two-power case
  drops the $x = 0$ that $x^q$ factors out; \verb+sqrt+, inverted by squaring, may return a root
  of the squared equation only. An assumption, then, not a verified selection: the residuals are
  the check.

  \medskip
  A block of several equations is reduced one unknown at a time, each step using
  the same rearrangements. A linear block is solved outright, by triangulation and
  back-substitution, which fails only when its determinant is zero.

\end{frame}


\begin{frame}[fragile]
  \frametitle{Steady state plan -- the rearrangements, toy examples --}

  The rearrangement is \verb+ast(residual).isolate(x)+, the equation being written
  as \verb+residual = 0+. On toy residuals it returns:
  ...
  \begin{tabular}{lll}
    ...
    \hline
    linear & \verb!2*x - 6! & \verb!x = 3! \\
           & \verb!a*x + b! & \verb!x = -(b / a)! \\
    one power & \verb!a*x^d - b! & \verb!x = (b / a) ^ (d ^ -1)! \\
    two powers & \verb!a*x^p - b*x^q! & \verb!x = (a / b) ^ ((q - p) ^ -1)! \\
    inside exp & \verb!exp(x) - y! & \verb!x = log(y)! \\
    inside log & \verb!log(x) - a! & \verb!x = exp(a)! \\
    inside sqrt & \verb!a*sqrt(x) - b! & \verb!x = (b / a) ^ 2! \\
    own lag & \verb!y - rho*y(-1) - e! & \verb!y = e / (1 - rho)! \\
    AR in logs & \verb!log(Z) - rho*log(Z) - e! & \verb!Z = exp(e / (1 - rho))! \\
    denominator & \verb!a/(x + 1) - b! & \verb!x = (a - b) / b! \\
    none & \verb!log(y) + y^3 - x! & \verb![]!, no closed form \\
    \hline
  \end{tabular}
  \end{center}

  \medskip
  \small The AR process in logs is shown in its static form: the plan makes every
  equation static before rearranging it, so that \verb+log(Z(-1))+ reads as
  \verb+log(Z)+.
...
\begin{frame}[c,fragile]
  \frametitle{Steady state plan -- the complex step --}

  Take an analytic $f$, a real point $x$, and step in the \textbf{imaginary}
  direction:
  \[
    f(x+\imath h) = f(x) + \imath h f'(x) - \frac{h^2}{2}f''(x)
    - \imath\frac{h^3}{6}f'''(x) + \dots
  \]
  The imaginary part holds the derivative alone, the even-order terms being real:
  \[
    \boxed{\;f'(x) = \frac{\Im\left(f(x+\imath h)\right)}{h} + O(h^2)\;}
  \]

  \medskip
  \small Nothing is \textbf{subtracted}. A finite difference $(f(x+h)-f(x))/h$
  differences two nearby numbers, so the digits they share are lost and $h$ cannot go
  below the floor where that cancellation overwhelms the $O(h)$ truncation -- around
  $\sqrt{\varepsilon}\simeq 10^{-8}$, leaving eight digits at best. Here there is no
  cancellation, so $h$ can be taken far below that floor: at $h=10^{-20}$ the
  truncation term is $O(10^{-40})$ and the derivative comes out exact to machine
  precision. On the residual of the next frame the complex step matches the symbolic
  derivative to $2$ ulps, where the best forward difference gives $2\cdot 10^{-8}$.

\end{frame}


\begin{frame}[c,fragile]
  \frametitle{Steady state plan -- the complex step, in the helper --}

  One residual evaluation per column, no derivative written to the file, and no
  dependency on modBuilder at run time -- here the stalled block of the running
  example, whose single unknown is \verb+i+:
...
if nargout > 1
  ...
  \small The step needs $f$ analytic \textbf{and} its \textsc{matlab} implementation
  to carry the imaginary part. It fails on \verb+abs+, which returns the modulus and
  silently destroys it, on \verb+sign+, \verb+min+, \verb+max+, not
  complex-differentiable, and on \verb+normcdf+, \verb+normpdf+, \verb+erf+,
  \verb+erfc+, \verb+cbrt+, whose implementations reject a complex argument. Powers,
  \verb+exp+, \verb+log+ and the trigonometric families are safe. \verb+Jacobian=auto+
  tests the block and falls back to the symbolic Jacobian; asking for
  \verb+complex-step+ on a block that fails the test is an error, not a silent
  downgrade.
...
\begin{frame}[fragile]
  \frametitle{Steady state plan -- when the plan stalls --}

  A fixed cost of production breaks the proportionality of output and consumption
  to hours:
...
b = model.steady_plan(Match=true, PropagateKnown=true);
  ...
  \begin{itemize}\small
  \item The plan stalls: \verb+h+, \verb+k+, \verb+i+, \verb+y+ and \verb+c+ form
    one block, four of them are expressed through \verb+i+, and \verb+i+ is left to
    a numerical solve. The equation left for it combines several powers of \verb+i+
    with the fixed cost, and none of the four rearrangements applies.
  \item Two ways out that stay symbolic: swap a role, pinning a variable and
    freeing a parameter (\verb+suggest_calibrations+); or declare a value the
    modeller already knows, and let \verb+suggest_anchors+ say which ones the plan
    needs.
  \end{itemize}
...
\begin{frame}[fragile]
  \frametitle{Steady state plan -- suggest\_calibrations --}

  First way out: swap a role, pinning a variable and freeing a parameter. On a
  stalled plan, \verb+print_steady_plan+ calls \verb+suggest_calibrations+ and prints
  the menu:
...
Suggested calibration swaps:
  ...
  \begin{itemize}\small
  \item Each swap is tried on a copy of the model; only those that shrink what is
    left open are listed, the ones that close the plan first.
  \item The variable left open, \verb+i+, has no parameter in its equation, the
    resource constraint: the scan then widens to every variable of the block.
  \item The menu is algebraic, not economic. Pinning hours and freeing $\theta$ is
    the textbook calibration: \verb+model.calibrate('h', 1/3, 'theta')+ closes the
    plan.
  \end{itemize}
...
\begin{frame}[fragile]
  \frametitle{Steady state plan -- suggest\_anchors --}

  Second way out: declare values the modeller already knows, here from a numerical
  solve, and let the plan say which it needs:
...
b = model.steady_plan(Match=true, Anchors=out.anchors, ...
  ...
  \begin{itemize}\small
  \item \verb+suggest_anchors+ tries dropping each declared value in turn, keeps only
    those the plan cannot do without, here output alone, and removes the
    declarations of the others (\verb+Apply=false+ only reports).
  \item With \verb+y+ as the only anchor, the plan gives \verb+i+, \verb+h+,
    \verb+c+ and \verb+k+ in closed form.
  \item An anchor frees one equation, which becomes a consistency condition the
    declared value must meet; it does here, the value coming from a numerical
    solve.
  \end{itemize}
...
\begin{frame}[c,fragile]
  ...
  \item \verb+scaling_symmetries+ finds these scale freedoms on its own, by homogeneity
    of the static system; the steady-state note covers it.
...
%==============================================================================
% Symbolic derivatives and optimal policy
%==============================================================================

\begin{frame}[c,fragile]
  \frametitle{Symbolic derivatives}

  \verb+partial+ differentiates the residual of one equation; here the equation keyed to
  \verb+c+, the capital accumulation, where \verb+k+ enters at two dates:
...
ans =
  ...
  \begin{itemize}\small
  \item Omitting \verb+Lag+ makes the equation static first, so the leads and lags of the
    variable aggregate -- the convention of \verb+isolate+ and of the AD Jacobian. Here
    $\delta = 1 + (\delta - 1)$.
  \item \verb+symbolic_jacobian(eqnames, varnames)+ returns the matrix of these partials as
    a cell of \verb+ast+ trees, sparse: \verb+isempty(J{i,j})+ is the test for a structural
    zero.
  \item Built on \verb+ast.diff_ast+ (see the AST note). This is the derivative of an
    equation, not the linearisation of the model -- that is \verb+tex_linearise+, later.
  \end{itemize}
...
\begin{frame}[c,fragile]
  \frametitle{Optimal policy -- the household --}
...
>> char(foc.foc)
  ...
  \small Marginal utility, labour supply, Euler. The derivation is written over
  \textbf{two periods}, as a Bellman problem, not as an infinite-sum Lagrangian: nothing
  assumes time-additivity, and that is why the multiplier carries a lead. \verb+augment+
  declares \verb+mult_1+ and keys the three equations, leaving a square block of five.

\end{frame}


\begin{frame}[c,fragile]
  \frametitle{Optimal policy -- the firm --}

  The firm maximises profit over its two inputs. There is no constraint to name, and the
...
>> char(ffoc.foc)
  ...
  \small The two factor demands: each marginal product equals its factor price. Nothing so
  far ties the firm's $kd$ and $hd$ to the household's $k$ and $h$ -- that is what the
  equilibrium does.
...
\begin{frame}[c,fragile]
  \frametitle{Optimal policy -- the equilibrium --}
...
>> m.equationmap()
  ...
  \small The textbook decentralised model, derived rather than typed. No resource
  constraint was added: it follows from the budget and zero profits. \verb+merge+ returns a
  new model, refuses two models sharing an endogenous variable, and converts \verb+h+ and
  \verb+k+ -- exogenous to the firm -- into the household's endogenous variables.

\end{frame}


\begin{frame}[c,fragile]
  \frametitle{Optimal policy -- Ramsey --}

  \verb+ramsey_foc+ is the other route: every equation but the value equation is a
  constraint, and the planner sets the instrument.
...
>> nk.equationmap()
  ...
  \small Six equations, no exogenous variable left: the instrument was promoted, and its
  own FOC pins $mult_2 = 0$. The FOCs are model equations like any other -- write them,
  render them with \verb+tex_model+, plan their steady state. Derive on a copy,
  \verb+m.copy().ramsey_foc(...)+, to leave the nonlinear model untouched.
