...
\begin{frame}[fragile]
  ...
  Suppose the modeller knows some steady-state values, here from a numerical solve:
...
out = model.suggest_anchors();      % anchors y, dropped h and k
  ...
  \begin{itemize}\small
  \item \verb+suggest_anchors+ tries dropping each declared value in turn, and keeps
    only those the plan cannot do without: here output alone.
  \item Declared alone and passed as the anchor, \verb+y+ closes the plan:
    \verb+steady_plan(Match=true, Anchors={'y'}, PropagateKnown=true)+ gives
    \verb+i+, \verb+h+, \verb+c+ and \verb+k+ in closed form.
  \item An anchor frees one equation, which becomes a consistency condition the
    ...
    solve.
  \item \verb+suggest_anchors+ only reports: the dropped values stay declared.
  \end{itemize}
