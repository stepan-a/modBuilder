...
\begin{frame}[fragile]
  ...
  \item it survives only as a common factor: \texttt{junk = 0.9*junk(+1)} reduces to
    \texttt{0.1*junk = 0}, which fixes \texttt{junk} at zero. The shape is ambiguous,
    though: an Euler equation leaves the same one, $c^{-\sigma} = \beta R\, c^{-\sigma}$,
    with the level of $c$ free. So the claim is weaker than in the first case;
