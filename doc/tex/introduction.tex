...
\begin{frame}[fragile]
  ...
  \item inside \verb+exp+ or \verb+log+: the function is inverted first, so
    $\log Z = \rho \log Z + e$ gives $Z = \exp\left(e/(1-\rho)\right)$.
  \end{itemize}
...
\begin{frame}[fragile]
  ...
  \verb+log(Z)+. \verb+sqrt+ is not inverted: written as a power,
  \verb+a*x^(1/2) - b+ gives \verb+x = (b / a) ^ 2+.
