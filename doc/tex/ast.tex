...
\begin{frame}\frametitle{Eight node types}
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    \code{expect} & dated expectation \code{EXPECTATION(k)(arg)} & \code{k} & \code{\{arg\}} \\
  ...
  \item \code{expect} carries \code{EXPECTATION(-1)(x(+1))} as written, for Dynare to
    expand; the steady state and the derivatives see through it.
...
\end{lstlisting}
  ...
  \item Disambiguation is by \textbf{name}: an identifier in
    ...
    \code{ss}; \code{EXPECTATION} is an \code{expect}; any other identifier followed
    by \code{(} expects a signed integer subscript.
  \end{itemize}
\end{frame}
...
\begin{frame}\frametitle{\texttt{isolate(x)}: closed form of $t = 0$ for $x$}
  ...
  \begin{enumerate}
  \item \textbf{invertible call} --- \code{exp}, \code{log} or \code{sqrt} (by
    squaring) wrapping $x$, recurse on the inverted equation. Several copies of the
    \emph{same} call are summed, so the static $\log Z - \rho\log Z - e$ closes to
    $Z = \exp(e/(1-\rho))$ --- not the raw $\log Z - \rho\log Z(-1) - e$, whose
    two calls differ;
  ...
  aggregated --- a bare $x(-1)$ at once, one inside a call once the residual is
  made static; other variables keep their lead/lag structure.
\end{frame}
...
\begin{frame}\frametitle{\texttt{to\_latex} and \texttt{complexity}}
  ...
  \item \code{tsym} \ra\ subscripts ($K(-1)\to K_{t-1}$); \code{ss} \ra\
    $\cdot^{\star}$; \code{expect} \ra\ $\mathbb{E}_{t+k}[\cdot]$; \code{exp} \ra\ $e^{\cdot}$;
    division \ra\ \code{\textbackslash frac};
...
\begin{frame}\frametitle{Summary}
  \begin{itemize}
  \item A value-class AST of eight node types, parsed by recursive descent.\newline
