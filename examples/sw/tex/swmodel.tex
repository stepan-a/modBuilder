...
\begin{equation}\label{eq:period_utility}
  u(C_t, L_t; \bm{\xi}_t)
  = \frac{\varsigma_t}{1-\sigma_c}
    \Bigl(C_t - \eta\,\eta^s_t\,\frac{C_{t-1}}{g_t}\Bigr)^{1-\sigma_c}
    \exp\!\Biggl(\frac{(\sigma_c - 1)\,\varepsilon^L_t}{1+\sigma_l}
    \Bigl(\frac{L_t}{L^{\star}}\Bigr)^{1+\sigma_l}\Biggr).
\end{equation}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Preference shock}: $\varsigma_t$ scales the period utility, and
    with it the marginal utility of both arguments. It enters multiplicatively,
    like every other exogenous process.
  \item \textbf{Non-separability}: when $\sigma_c \neq 1$, the marginal
    ...
    $(L_t/L^{\star})^{1+\sigma_l}$ enter multiplicatively, generating a wealth
...
\begin{equation}\label{eq:adj_cost}
  ...
  = \frac{\psi}{2}\,(1+g^{\star})^2
    \left(\frac{I_t}{I_{t-1}}\,\frac{g_t}{1+g^{\star}} - 1\right)^{\!2},
\end{equation}
with derivative
\begin{equation}\label{eq:adj_cost_deriv}
  S'_t \equiv \frac{\partial S}{\partial (I_t g_t / I_{t-1})}
  = \psi\,(1+g^{\star})
    \left(\frac{I_t}{I_{t-1}}\,\frac{g_t}{1+g^{\star}} - 1\right).
\end{equation}
At steady state, $I_t/I_{t-1} = 1$ and $g_t = 1+g^{\star}$, so
...
\begin{equation}\label{eq:depreciation}
  \delta(z_t) = \delta^{\star} \, \exp\!\left(
    \frac{{\delta'}^{\star}}{\delta^{\star}}\,(z_t - z^{\star})\right),
\end{equation}
where $\delta^{\star} = \delta(z^{\star})$ is the steady-state depreciation rate and
${\delta'}^{\star} = \delta'(z^{\star})$ is the steady-state marginal depreciation.
...
\begin{equation}\label{eq:eq1}\tag{Eq.~1}
  \boxed{
  \varsigma_t\,\bigl(C_t - \eta\,\eta^s_t\,C_{t-1}/g_t\bigr)^{-\sigma_c}
  \exp\!\Bigl(\tfrac{(\sigma_c-1)\varepsilon^L_t}{1+\sigma_l}
  (L_t/L^{\star})^{1+\sigma_l}\Bigr)
  = \tau^C_t\,\lambda_t.}
\end{equation}
...
\begin{equation}\label{eq:eq3}\tag{Eq.~3}
  \boxed{
  \begin{aligned}
  &\varsigma_t\,\bigl(C_t - \eta\,\eta^s_t\,C_{t-1}/g_t\bigr)^{1-\sigma_c}
  \,\frac{\varepsilon^L_t}{L^{\star}}
  \,\Bigl(\frac{L_t}{L^{\star}}\Bigr)^{\!\sigma_l} \\
...
\begin{equation}\label{eq:eq5}\tag{Eq.~5}
  \boxed{
  \delta_t = \delta^{\star}\,
  \exp\!\Bigl(\frac{{\delta'}^{\star}}{\delta^{\star}}(z_t - z^{\star})\Bigr).}
\end{equation}
...
\begin{equation}\label{eq:eq10}\tag{Eq.~10}
  \boxed{
  S_t = \frac{\psi}{2}\,(1{+}g^{\star})^2
    \Bigl(\frac{I_t}{I_{t-1}}\,\frac{g_t}{1{+}g^{\star}} - 1\Bigr)^{\!2}.}
\end{equation}
...
\begin{equation}\label{eq:eq11}\tag{Eq.~11}
  \boxed{
  S'_t = \psi\,(1{+}g^{\star})
    \Bigl(\frac{I_t}{I_{t-1}}\,\frac{g_t}{1{+}g^{\star}} - 1\Bigr).}
\end{equation}
...
($\bbar\pi^{\star} = 1$) and a unit growth rate ($g^{\star} = 1$); the Kimball
...
\begin{itemize}[nosep]
  ...
  \item $R^{\star}$ is the steady-state nominal interest rate;
...
$X^{\star}$, autocorrelation parameter $\rho_X$, and innovation
...
shock. In log deviations $\hat x_t = \ln(X_t/X^{\star})$,
...
\[
  \frac{X_t}{X^{\star}}
  = \prod_{i=1}^{p}\Big(\frac{X_{t-i}}{X^{\star}}\Big)^{\!\phi_i}
...
\begin{equation}\tag{Eq.~60}
  \varsigma_t\,\bigl(C^e_t - \eta\,\eta^s_t\,C^e_{t-1}/g_t\bigr)^{-\sigma_c}
  \exp\!\Bigl(\tfrac{(\sigma_c-1)\varepsilon^L_t}{1+\sigma_l}
  (L^e_t/L^{\star})^{1+\sigma_l}\Bigr)
  = \tau^C_t\,\lambda^e_t.
\end{equation}
...
\begin{enumerate}[nosep]
  \item The \textbf{discount factor} is pinned down by the Euler
    ...
    \[
      \beta = \frac{\bbar\pi^{\star}}{{\varepsilon^B}^{\star}\,R^{\star}}\,
      (1+g^{\star})^{\sigma_c}.
    \]
  \item The \textbf{steady-state capital rental rate} from the capital
    FOC~\eqref{eq:eq8}:
    \[
      {r^k}^{\star} = \frac{(1+g^{\star})^{\sigma_c} - \beta(1-\delta^{\star})}
      {\beta\,{\tau^R}^{\star}\,z^{\star}}.
    \]
  \item The \textbf{capital share} from the production function:
    \[
      \alpha = 1 - {\tau^L}^{\star}\,\mu_f\,\frac{w^{\star}\,L^{\star}}{Y^{\star}}.
    \]
  \item The \textbf{steady-state gross wage}:
    \[
      w^{\star} = \frac{(1-\alpha)}{{\tau^L}^{\star}}\,
      \Bigl(\frac{1}{\mu_f}\,
      \Bigl(\frac{\alpha}{{r^k}^{\star}}\Bigr)^{\!\alpha}\Bigr)^{\!1/(1-\alpha)}\,
      A^{\star}.
    \]
  \item The \textbf{investment-to-GDP ratio}:
    \[
      \frac{I^{\star}}{Y^{\star}}
      = \frac{g^{\star} + \delta^{\star}}{z^{\star}}\,
        \frac{(\bbar K^{\star}/L^{\star})^{1-\alpha}}{\left.A^{\star}\right.^{1-\alpha}},
    \]
    where $\bbar K^{\star}/L^{\star} = {\tau^L}^{\star}\,w^{\star}\,\alpha/[(1-\alpha){r^k}^{\star}]$.
  \item The \textbf{consumption-to-GDP ratio}:
    \[
      \frac{C^{\star}}{Y^{\star}}
      = 1 - {\varepsilon^g}^{\star} - \frac{I^{\star}}{Y^{\star}}.
    \]
\end{enumerate}
