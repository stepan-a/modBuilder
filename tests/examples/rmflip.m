% Test examples from rmflip method documentation

addpath ../utils

% Example 1: Remove y's equation, make k exogenous instead
m = modBuilder();
m.add('y', 'y = a*k');
m.add('k', 'k = (1-delta)*k(-1) + i + y');
m.parameter('a', 0.33);
m.parameter('delta', 0.025);
m.exogenous('i', 0);

% Verify initial state
if ~m.isendogenous('y')
    error('y should be endogenous initially')
end
if ~m.isendogenous('k')
    error('k should be endogenous initially')
end

% Remove y's equation, make k exogenous instead
m.rmflip('y', 'k');
% y stays endogenous (determined by k's former equation), k becomes exogenous

% Verify result
if ~m.isendogenous('y')
    error('y should be endogenous after rmflip')
end
if ~m.isexogenous('k')
    error('k should be exogenous after rmflip')
end
if m.size('endogenous') ~= 1
    error('Model should have 1 endogenous variable after rmflip')
end

fprintf('Example 1 passed: Remove y''s equation, make k exogenous\n');

fprintf('rmflip.m: All tests passed\n');
