...
% Example 1: Simple substitution (backward compatible)
m = modBuilder();
...
if ~strcmp(m{'Y'}.equations{2}, 'Y = gamma*K + beta*L')
    error('Example 1 failed: simple substitution')
end

fprintf('Example 1 passed: Simple substitution\n');

% Example 2: Implicit loop - substitute in all equations
m2 = modBuilder();
m2.add('Y', 'Y = alpha_1*K_1 + alpha_2*K_2 + alpha_3*K_3');
m2.parameter('alpha_$1', 0.3, {1, 2, 3});
m2.exogenous('K_$1', 1.0, {1, 2, 3});

m2.subs('alpha_$1', 'beta_$1', {1, 2, 3});
...
% Example 3: Implicit loop - substitute in specific equation
m3 = modBuilder();
...
m3.subs('alpha_$1', 'beta_$1', 'Y', {1, 2});
...
% Example 4: Implicit loop with explicit equation name
m4 = modBuilder();
...
m4.subs('alpha_$1', 'gamma_$1', 'Y', {1, 2});
