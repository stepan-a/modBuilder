% Test examples from subs method documentation

addpath ../utils

% Example 1: Substitute a defining variable everywhere, then drop the now-tautological equation.
m = modBuilder();
m.add('Y', 'Y = mc * X');
m.add('mc', 'mc = w / mpl');
m.exogenous('X', 1);
...
fprintf('Example 1 passed: Substitute a defining variable, then remove its equation\n');

% Example 2: Substitute only into a specific equation, with a parameter in the replacement.
% theta is introduced into the model by the substitution, then promoted to a parameter.
m = modBuilder();
m.add('Y', 'Y = mc * X');
m.add('mc', 'mc = w / mpl');
m.exogenous('X', 1);
...
fprintf('Example 2 passed: Substitute into a specific equation with a parameter in the replacement\n');

% Example 3: Implicit loop — substitute alpha_i by a constant in every equation.
m = modBuilder();
...
fprintf('subs.m: All tests passed\n');
