...
% inline: implicit loop with one $ placeholder, no eqname (apply to all equations).
...
if not(ast.ast_equal(got_rhs, expected_rhs))
    error('inline: implicit-loop expansion failed. expected "%s", got "%s"', expected_rhs.string(), got_rhs.string())
end
