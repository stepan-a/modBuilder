...
% inline: precedence is preserved through substitution, fixing the strrep bug.
...
if not(ast.ast_equal(got_rhs, expected_rhs))
    error('inline: precedence not preserved. expected "%s", got "%s"', expected_rhs.string(), got_rhs.string())
end
