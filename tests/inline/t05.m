...
% inline: word-boundary safety. Substituting "k" must not touch "k_bar".
...
if not(ast.ast_equal(got_rhs, expected_rhs))
    error('inline: word boundary violated. expected "%s", got "%s"', expected_rhs.string(), got_rhs.string())
end
...
if m.issymbol('k')
    error('k should be removed after inline')
end
