% Tests for substitute errors (regex-based substitution)
...
thrown = false;
try
    m.substitute('x', 'z', 'y', 'c');
catch
    thrown = true;
end
assert(thrown, 'Expected error: only one equation name allowed.');

% Test 2: Index values provided but no placeholders
thrown = false;
try
    m.substitute('x', 'z', {1, 2});
catch
    thrown = true;
end
assert(thrown, 'Expected error: no placeholders but index values provided.');

% Test 3: Index placeholder mismatch between expr1 and expr2
thrown = false;
try
    m.substitute('x_$1', 'z_$2', {1, 2});
catch
    thrown = true;
end
...
thrown = false;
try
    m2.substitute('x_$1_$2', 'z_$1_$2', {1, 2});
catch
    thrown = true;
end
