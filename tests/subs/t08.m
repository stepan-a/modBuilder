...
% subs: implicit loop with two placeholders in varname/replacement.
...
if not(ast.ast_equal(got_rhs, expected_rhs))
    error('subs: two-index expansion failed. expected "%s", got "%s"', expected_rhs.string(), got_rhs.string())
end
